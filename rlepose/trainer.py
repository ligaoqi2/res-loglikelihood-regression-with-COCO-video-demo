...
def validate(m, opt, cfg, heatmap_to_coord, batch_size=20, use_nms=False):
    ...
    for inps, crop_bboxes, bboxes, img_ids, scores, imghts, imgwds in det_loader:
        ...
        if opt.flip_test:
            ...
            output_flipped = flip_output(m(inps_flipped), det_dataset.joint_pairs, width_dim=hm_size[1], shift=flip_shift)
...
def validate_gt(m, opt, cfg, heatmap_to_coord, batch_size=20):
    ...
    for inps, labels, img_ids, bboxes in gt_val_loader:
        ...
        # inps -> Tensor (64, 3, 256, 192)
        # output easy dict
        # {'maxvals' -> Tensor (64, 17, 1), 'pred_jts' -> Tensor (64, 17, 2), 'sigma' -> Tensor (64, 17, 2), 'nf_loss' -> None}
        # opt.flip_test -> true
        ...
        for i in range(inps.shape[0]):
            ...
            # bboxes tensor (64, 4) [xmin, ymin, xmax, ymax]
            # bbox list 4 float
            pose_coords, pose_scores = heatmap_to_coord(
                output, bbox, idx=i)

            # pose_coords -> Array (1, 17, 2)
            # pose_scores -> Array (1, 17, 1)
...
def validate_gt_3d(m, opt, cfg, heatmap_to_coord, batch_size=20):
    gt_val_dataset = builder.build_dataset(cfg.DATASET.VAL, preset_cfg=cfg.DATA_PRESET, train=False, heatmap2coord=cfg.TEST.HEATMAP2COORD)
    gt_val_sampler = torch.utils.data.distributed.DistributedSampler(gt_val_dataset, num_replicas=opt.world_size, rank=opt.rank)

    gt_val_loader = torch.utils.data.DataLoader(gt_val_dataset, batch_size=batch_size, shuffle=False, num_workers=20, drop_last=False, sampler=gt_val_sampler)
    kpt_pred = {}
    m.eval()
